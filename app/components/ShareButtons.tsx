...
export default function SocialShare({
  title = `${CONSTANTS.BRAND_NAME} - Best IPTV Provider Australia 2026`,
  ...
  return (
    <div className="w-full flex justify-center items-center px-4 my-8 relative z-10">
      <div className="w-full max-w-4xl bg-[#0a0a0c]/80 backdrop-blur-xl border border-white/15 rounded-2xl md:rounded-full py-3.5 px-6 sm:px-8 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-xl transition-colors duration-300 hover:border-[#00843D]/60">

        {/* Callout Header */}
        <div className="flex items-center justify-center gap-2.5 text-white shrink-0">
          <span className="p-2 rounded-full bg-[#00843D] text-[#FFCD00]">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 h-10 sm:h-11 rounded-full text-xs font-black uppercase tracking-wider border transition-all duration-200 active:scale-95 cursor-pointer shadow-sm ${
              copied
                ...
                : 'bg-[#FFCD00] border-[#FFCD00] text-[#00662F] hover:bg-[#00843D] hover:text-white hover:border-[#00843D]'
            }`}
          >
            {copied ? (
              <>
